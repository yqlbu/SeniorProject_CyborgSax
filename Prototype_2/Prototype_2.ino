...
int buttonPushpos = 0;   // pos for the number of button presses
...
AudioPlaySdWav           playSdWav1;     //xy=98,193
AudioInputI2S            Mic;           //xy=106,111
...
AudioConnection          patchCord3(Mic, 0, mixer1, 0);
AudioConnection          patchCord4(Mic, 1, mixer1, 1);
AudioConnection          patchCord5(mixer1, amp1);
AudioConnection          patchCord6(amp1, 0, i2s1, 0);
AudioConnection          patchCord7(amp1, 0, i2s1, 1);
AudioConnection          patchCord8(amp1, fft1024_1);
...
void loop() {
  count++;
  pos = constrain(pos, 0, 5); //set the constrain to the volume
  amp1.gain(pos);
  ...
  if (buttonState != lastButtonState) {
    // if the state has changed, increment the pos
    if (buttonState == HIGH) {
      // if the current state is HIGH then the button went from off to on:
      buttonPushpos = (buttonPushpos + 1) % 5;
      color_choose = buttonPushpos;
        Serial.println("button pushed!");
        Serial.print("mode: ");
        Serial.println(buttonPushpos);
   }
  }
